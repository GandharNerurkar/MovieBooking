    ...
    <div key={row} className="flex gap-2 mt-2">
      <div className="flex flex-wrap items-center justify-center gap-2">
        {Array.from({ length: count }, (_, i) => {
          ...
          return <button key={seatId} onClick={() => handleSeatClick(seatId)} className={`h-8 w-8 rounded border border-primary/60 cursor-pointer ${selectedSeats.includes(seatId) && "bg-primary text-white"}`}>
            {seatId}
          </button>;
        })}
      </div>
    ...
    <div className="flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50">
      ...
      <div className="relative flex-1 flex flex-col items-center max-md:mt-16">
        ...
        <div className="flex flex-col items-center mt-10 text-xs text-gray-300">
          <div className="grid grid-col-2 md:grip-cols-1 gap-8 md:gap-2 mb-6">{groupRows[0].map(row=> renderSeats(row))}</div>
